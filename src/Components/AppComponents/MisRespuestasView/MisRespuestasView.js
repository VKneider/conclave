...
export default class MisRespuestasView extends HTMLElement {
  constructor(props) {
    ...
    this.$carouselSlot = this.querySelector('.mrv-carousel-slot');
    this.$dotsNav = this.querySelector('#dotsNav');
  ...
  async _render() {
    ...
    if (!list.length) {
      this.$carouselSlot.innerHTML = this._html.sanitize('<div class="empty-state">No hay opciones que coincidan.</div>');
    ...
    this.$carouselSlot.innerHTML = this._html.sanitize(`
        ...
        <div class="person-card">
          ...
          <div class="tema-pills">
            ${temas.map((t) => {
              ...
              return `<button class="pill ${sel === t.id ? 'selected' : ''} ${atCapacity ? 'at-capacity' : ''}" data-tema="${t.id}">
                ${this._html.esc(t.nombre)} <span class="cap">${n}/${t.max != null ? t.max : '–'}</span>
              </button>`;
            }).join('')}
            <button class="pill pill-clear" data-tema="">✕ Sin asignar</button>
          </div>
        </div>
        <button class="arrow-btn" data-act="next" ${this.carouselIndex === list.length - 1 ? 'disabled' : ''}>›</button>
      </div>`);

    this.$dotsNav.innerHTML = this._html.sanitize(list.map((m, i) => {
      const cls = ['dot'];
      if (asignaciones[m.id]) cls.push('done');
      if (i === this.carouselIndex) cls.push('active');
      return `<span class="${cls.join(' ')}" data-idx="${i}" title="${this._html.esc(m.nombre)}"></span>`;
    }).join(''));
  ...
  _bindDots() {
    this.$dotsNav.querySelectorAll('.dot').forEach((d) => {
