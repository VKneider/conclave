...
> **Also read** `docs/COMPONENT-PATTERNS.md` §UI patterns for the interaction patterns that are not animation: **fullscreen overlays** (⛶ text editor/reader), **Enter-plus-tap-button add rows** (mobile), the shared **`Tabs`** segmented control (kind/mode tabs) and the **hold-to-drag** touch contract (GOTCHAS §42). All Spanish UI copy is **neutral (tuteo)** — no voseo (`arma`/`elige`/`escribe`, not `armá`/`elegí`/`escribí`).
...
| Drag ghost (PorTemaView) | real-time | — | Pointer follows finger. No easing — direct 1:1. |
| Pulsing outline (over-capacity) | 2.4s | ease-in-out | `.ps-square.is-over` in PorTemaView. Disabled under `prefers-reduced-motion`. |
...
1. User clicks a Tema pill → `_advancePending = true`, `update()` called.
2. `_render()` paints the carousel with the current Opción (not yet advanced).
3. After binding, `_showAdvanceFeedback()` runs:
   ...
   - **Summary**: `.assign-summary` text set to `"OpciónName → TemaName"`, `.visible` class added → `summarySlideIn` plays.
   - **Duration**: 500ms timeout.
4. After 500ms: `_advancePending = false`, `carouselIndex++`, `update()` repaints the next Opción.
...
Replaced the old CSV/JSON textarea + bulk diff-bar preview entirely — Temas
and Opciones are now edited as individual `TemaRow`/`OpcionRow` components, each with its own
...
limpiarán N respuestas que apuntaban a este tema"`) — the same information the old
diff bar used to summarize in aggregate, just computed per single deletion
instead of per bulk save.

Each row component carries the same dense-row treatment in its own CSS
(`.cat-row` in `TemaRow.css`, `.opc-row` in `OpcionRow.css`):

```css
slice-temarow .cat-row {
  border: 1.5px solid var(--border-color); border-radius: 10px; padding: 10px 12px;
  transition: border-color .12s, box-shadow .12s;
}
slice-temarow .cat-row:hover,
slice-temarow .cat-row:focus-within {
  border-color: var(--font-primary-color);
  box-shadow: 2px 2px 0 var(--font-primary-color);
}
```

`AtributoRow` is the exception: it sits inside an already-bordered sub-panel,
so it carries no border or shadow of its own.
...
card per author for the active Tema, `17px` line-height-1.6 body text,
`5px 5px 0` shadow. Navigation is by Tema (prev/next), not by author —
...
- PorTemaView: pulsing outline (2.4s) on `.is-over` squares.
